...
func _ready():
	...
	$CardBack/DoneSprite.scale *= card_size/$CardBack/DoneSprite.texture.get_size()
...
# Automatically draw next card in deck after selecting card answer
func draw_next_card():
	var draw_deck = get_parent().get_parent().get_parent().get_node("CardScene/draw_deck")
	# Expects 3 children (current answered card, card to draw, and empty deck placeholder):
	if draw_deck.get_child_count() >= 3:
		draw_deck.get_child(1)._on_BtnDraw_button_up()
